...
Public Class modelDefect
    ...
    Public Shared Function mGetDataSupplier(item_cd As String)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/API_GEMBA/GET_OEE/EXP_CHECK_SUPP?item_cd=" & item_cd)
            Console.WriteLine("http://" & Backoffice_model.svApi & "/API_GEMBA/GET_OEE/EXP_CHECK_SUPP?item_cd=" & item_cd)
            Return rsData
    ...
    Public Shared Function minsertDefectTrascetionSupplier(lastId As String, dt_supplier_code As String, dt_qty As String, Line As String)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/insertDatadefect/insertDefectTransctionSupplier?lastId=" & lastId & "&dt_supplier_code=" & dt_supplier_code & "&dt_qty=" & dt_qty & "&Line=" & Line)
            Console.WriteLine("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/insertDatadefect/insertDefectTransctionSupplier?lastId=" & lastId & "&dt_supplier_code=" & dt_supplier_code & "&dt_qty=" & dt_qty & "&Line=" & Line)
            Return rsData
        Catch ex As Exception
    ...
    Public Shared Function mGetPwi_id(WI As String, LOT_NO As String, SEQ_NO As String, SHIFT As String)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/getPwi?WI=" & WI & "&LOT_NO=" & LOT_NO & "&SEQ_NO=" & SEQ_NO & "&SHIFT=" & SHIFT)
            Console.WriteLine("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/getPwi?WI=" & WI & "&LOT_NO=" & LOT_NO & "&SEQ_NO=" & SEQ_NO & "&SHIFT=" & SHIFT)
            If rsData <> "0" Then
                Return rsData
            Else
    ...
    Public Shared Function mGetTagprintDetailSpecial(wi As String, lot As String, seq As String, shift As String)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/GetTagprintDetailSpecial?wi=" & wi & "&lot=" & lot & "&seq=" & seq & "&shift=" & shift)
            Console.WriteLine("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/GetTagprintDetailSpecial?wi=" & wi & "&lot=" & lot & "&seq=" & seq & "&shift=" & shift)
            If rsData <> "0" Then
                Return rsData
            Else
    ...
    Public Shared Function mGetDataEnableFGPart(line_cd As String)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/GetDataEnableFGPart?line_cd=" & line_cd)
            Console.WriteLine("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/GetDataEnableFGPart?line_cd=" & line_cd)
            Return rsData
        Catch ex As Exception
    ...
    Public Shared Function mGetGoodWILot(wi As String, lot_no As String)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/GetGoodWILot?wi=" & wi & "&lot_no=" & lot_no)
            Console.WriteLine("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/GetGoodWILot?wi=" & wi & "&lot_no=" & lot_no)
            Return rsData
        Catch ex As Exception
    ...
    Public Shared Function mGetTagprintDetailNormal(wi As String, lot As String, seq As String, shift As String)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/GetTagprintDetailNormal?wi=" & wi & "&lot=" & lot & "&seq=" & seq & "&shift=" & shift)
            Console.WriteLine("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/GetTagprintDetailNormal?wi=" & wi & "&lot=" & lot & "&seq=" & seq & "&shift=" & shift)
            If rsData <> "0" Then
                Return rsData
            Else
    ...
    Public Shared Function mGetchildpart(wi)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/getChildpart?wi=" & wi)
            Console.WriteLine("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/getChildpart?wi=" & wi)
            If rsData <> "0" Then
                Return rsData
            Else
    ...
    Public Shared Function mGetDatamsterLine(line As String)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/GetDataLine?line=" & line)
            If rsData <> "0" Then
                Return rsData
            Else
    ...
    Public Shared Function mGetchildpartSpc(arrWi As Array)
        Try
            ...
            Dim url As String = "http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefectSpecial/getChildpart"
    ...
    Public Function mGetPartno(wi As String)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/GetMainPartNo?Wi=" & wi)
            If rsData <> "0" Then
                Return rsData
            Else
    ...
    Public Function mGetdatepartdetail(pNo As String, flg As String)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/getDataplan?itemCd=" & pNo & "&flg=" & flg)
            Console.WriteLine("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/getDataplan?itemCd=" & pNo & "&flg=" & flg)
            If rsData <> "0" Then
                Return rsData
            Else
    ...
    Public Shared Function mGetdefectcode(LineCd As String)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/getDefectcode?LineCd=" & LineCd)
            If rsData <> "0" Then
                Return rsData
            Else
    ...
    Public Shared Function mGetDatadefectcodeprint(wi As String, lot As String, seqNo As String, itemCd As String, dfType As String)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/getDatadefectcodeprint?wi=" & wi & "&lot=" & lot & "&seqNo=" & seqNo & "&itemCd=" & itemCd & "&dfType=" & dfType)
            'MsgBox("model====>" & rsData)
            Console.WriteLine("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/getDatadefectcodeprint?wi=" & wi & "&lot=" & lot & "&seqNo=" & seqNo & "&itemCd=" & itemCd & "&dfType=" & dfType)
            If rsData <> "0" Then
                Return rsData
            Else
    ...
    Public Shared Function mGetboxInformation(wi As String, lot As String, seqNo As String)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/getBoxInformation?wi=" & wi & "&lot=" & lot & "&seq=" & seqNo)
            If rsData <> "0" Then
                Return rsData
            Else
    ...
    Public Shared Function mInsertdefectregister(dtWino As String, dtLineno As String, dtItemcd As String, dtItemtype As String, dtLotno As String, dtSeqno As String, dtType As String, dtCode As String, dtQty As String, dtMenu As String, dtActualdate As String, pwi_id As String)
        Try
            Dim api = New api()
            Dim rsData As Boolean = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/insertDatadefect/insertDefectregister?dtWino=" & dtWino & "&dtLineno=" & dtLineno & "&dtItemcd=" & dtItemcd & "&dtItemtype=" & dtItemtype & "&dtLotNo=" & dtLotno & "&dtSeqno=" & dtSeqno & "&dtType=" & dtType & "&dtCode=" & dtCode & "&dtQty=" & dtQty & "&dtMenu=" & dtMenu & "&dtActualdate=" & dtActualdate & "&pwi_id=" & pwi_id)
            Console.WriteLine("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/insertDatadefect/insertDefectregister?dtWino=" & dtWino & "&dtLineno=" & dtLineno & "&dtItemcd=" & dtItemcd & "&dtItemtype=" & dtItemtype & "&dtLotNo=" & dtLotno & "&dtSeqno=" & dtSeqno & "&dtType=" & dtType & "&dtCode=" & dtCode & "&dtQty=" & dtQty & "&dtMenu=" & dtMenu & "&dtActualdate=" & dtActualdate & "&pwi_id=" & pwi_id)
            Return rsData
        Catch ex As Exception
    ...
    Public Shared Function mInsertdefectactual(dtWino As String, dtLineno As String, dtItemcd As String, dtItemtype As String, dtLotno As String, dtSeqno As String, dtType As String, dtCode As String, dtQty As String, dtMenu As String, dtActualdate As String, pwi_id As String)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/insertDatadefect/insertDefectactual?dtWino=" & dtWino & "&dtLineno=" & dtLineno & "&dtItemcd=" & dtItemcd & "&dtItemtype=" & dtItemtype & "&dtLotNo=" & dtLotno & "&dtSeqno=" & dtSeqno & "&dtType=" & dtType & "&dtCode=" & dtCode & "&dtQty=" & dtQty & "&dtMenu=" & dtMenu & "&dtActualdate=" & dtActualdate & "&pwi_id=" & pwi_id)
            Console.WriteLine("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/insertDatadefect/insertDefectactual?dtWino=" & dtWino & "&dtLineno=" & dtLineno & "&dtItemcd=" & dtItemcd & "&dtItemtype=" & dtItemtype & "&dtLotNo=" & dtLotno & "&dtSeqno=" & dtSeqno & "&dtType=" & dtType & "&dtCode=" & dtCode & "&dtQty=" & dtQty & "&dtMenu=" & dtMenu & "&dtActualdate=" & dtActualdate & "&pwi_id=" & pwi_id)
            Return rsData
        Catch ex As Exception
    ...
    Public Shared Function mUpdateaddjust(dtWino As String, dtLotNo As String, dtSeqno As String, dtType As String, dtCode As String, ItemType As String, PartNo As String)
        Try
            ...
            Dim api = New api()
            Dim rsData As Boolean = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/updateDatadefect/updateDatadefectregister?dtWino=" & dtWino & "&dtLotNo=" & dtLotNo & "&dtSeqno=" & CDbl(Val(dtSeqno)) & "&dtType=" & dtType & "&dtCode=" & dtCode & "&dtItemType=" & ItemType & "&PartNo=" & PartNo)
            Return rsData
        Catch ex As Exception
    ...
    Public Shared Function mUpdatedefectactual(dtWino As String, dtLotNo As String, dtSeqno As String, dtType As String, dtCode As String)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/updateDatadefect/updateDefectactual?dtWino=" & dtWino & "&dtLotNo=" & dtLotNo & "&dtSeqno=" & dtSeqno & "&dtType=" & dtType & "&dtCode=" & dtCode & "&itemCd=" & itemCd)
            Return rsData
        Catch ex As Exception
    ...
    Public Shared Function mUpdatedefectactualAdmin(dtWino As String, dtLotNo As String, dtSeqno As String, dtType As String, dtCode As String, dtItemType As String, ItemCd As String)
        Try
            Dim api = New api()
            Console.WriteLine("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/updateDatadefect/updateDefectactualAdmin?dtWino=" & dtWino & "&dtLotNo=" & dtLotNo & "&dtSeqno=" & dtSeqno & "&dtType=" & dtType & "&dtCode=" & dtCode & "&dtItemType=" & dtItemType & "&ItemCd=" & ItemCd)
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/updateDatadefect/updateDefectactualAdmin?dtWino=" & dtWino & "&dtLotNo=" & dtLotNo & "&dtSeqno=" & dtSeqno & "&dtType=" & dtType & "&dtCode=" & dtCode & "&dtItemType=" & dtItemType & "&ItemCd=" & ItemCd)
            Return rsData
        Catch ex As Exception
            MsgBox("connect Api Faill Please check modelDefect in Function mUpdatedefectactual = " & ex.Message)
            Return False
    ...
    Public Shared Function mGetdefectdetailnc(dtWino As String, dtSeq As String, dtLot As String, Type As String)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/getDefectnc?dfWi=" & dtWino & "&dfSeq=" & dtSeq & "&dfLot=" & dtLot & "&dfType=" & Type)
            Console.WriteLine("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/getDefectnc?dfWi=" & dtWino & "&dfSeq=" & dtSeq & "&dfLot=" & dtLot & "&dfType=" & Type)
            If rsData <> "0" Then
                Return rsData
            Else
                Return False
    ...
    Public Shared Function mGetdefectdetailncSpc(arrayWI As Array, lengthDataPlan As Integer, dtLot As String, Type As String, startSeq As Integer)
        Try
            ...
            Dim url As String = "http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefectSpecial/getDefectnc"
    ...
    Public Shared Function mGetdefectdetailncPartno(dtWino As String, dtSeq As String, dtLot As String, Type As String, PartNo As String)
        Try
            Dim api = New api()
            Console.WriteLine("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/getDefectncPartNo?dfWi=" & dtWino & "&dfSeq=" & dtSeq & "&dfLot=" & dtLot & "&dfType=" & Type & "&PartNo=" & PartNo)
            Dim rsData As String = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/getDefectncPartNo?dfWi=" & dtWino & "&dfSeq=" & dtSeq & "&dfLot=" & dtLot & "&dfType=" & Type & "&PartNo=" & PartNo)
            If rsData <> "0" Then
                Return rsData
            Else
                Return 0
    ...
    Public Shared Function mGetdefectdetailPartno(dtWino As String, dtSeq As String, dtLot As String, Type As String, PartNo As String)
        Try
            Dim api = New api()
            Console.WriteLine("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/getDefectPartNo?dfWi=" & dtWino & "&dfSeq=" & dtSeq & "&dfLot=" & dtLot & "&dfType=" & Type & "&PartNo=" & PartNo)
            Dim rsData As String = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/getDefectPartNo?dfWi=" & dtWino & "&dfSeq=" & dtSeq & "&dfLot=" & dtLot & "&dfType=" & Type & "&PartNo=" & PartNo)
            If rsData <> "0" Then
                Return rsData
            Else
                Return 0
    ...
    Public Shared Function mGetdefectdetailncPartnoSpc(arrayWI As Array, lengthDataPlan As Integer, dtLot As String, Type As String, PartNo As String, dfWiSel As String, dfSeqSel As String)
        Try
            ...
            Dim url As String = "http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefectSpecial/getDefectncPartNo"
            Dim rsData = api.Load_dataPOST(url, requestFunction)
            If rsData <> "0" Then
                Return rsData
            Else
    ...
    Public Shared Function mGetdatachildpartsummarychild(dtWino As String, dtSeq As String, dtLot As String)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/Getdatachildpartsummarychild?dfWi=" & dtWino & "&dfSeq=" & dtSeq & "&dfLot=" & dtLot)
            If rsData <> "0" Then
                Return rsData
            Else
                Return "0"
    ...
    Public Shared Function mGetdatachildpartsummarychildSpc(arrayWI As Array, lengthDataPlan As Integer, dtLot As String, startseq As Integer)
        Try
            ...
            requestFunction("dfLot") = dtLot
            requestFunction("startseq") = startseq
            Dim url As String = "http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefectSpecial/GetdatachildpartsummarychildSpc"
            Dim result = api.Load_dataPOST(url, requestFunction)
            'Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefectSpecial/GetdatachildpartsummarychildSpc?dfWi1=" & dtWino1 & "&dfWi2=" & dtWino2 & "&dfWi3=" & dtWino3 & "&dfWi4=" & dtWino4 & "&dfWi5=" & dtWino5 & "&dfSeq1=" & dtSeq1 & "&dfSeq2=" & dtSeq2 & "&dfSeq3=" & dtSeq3 & "&dfSeq4=" & dtSeq4 & "&dfSeq5=" & dtSeq5 & "&dfLot=" & dtLot)
            If result <> "0" Then
                Return result
            Else
                Return "0"
    ...
    Public Shared Function mGetdatachildpartsummaryfg(dtWino As String, dtSeq As String, dtLot As String)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/Getdatachildpartsummaryfg?dfWi=" & dtWino & "&dfSeq=" & dtSeq & "&dfLot=" & dtLot)
            If rsData <> "0" Then
                Return rsData
            Else
                Return "0"
    ...
    Public Shared Function mGetdatachildpartsummaryfgSpc(arrayWI As Array, lengthDataPlan As Integer, dtLot As String, startseq As Integer)
        Try
            ...
            requestFunction("dfLot") = dtLot
            requestFunction("startseq") = startseq
            Dim url As String = "http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefectSpecial/GetdatachildpartsummaryfgSpc"
            Dim rsData = api.Load_dataPOST(url, requestFunction)
            '  Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefectSpecial/GetdatachildpartsummaryfgSpc?dfWi1=" & dtWino1 & "&dfWi2=" & dtWino2 & "&dfWi3=" & dtWino3 & "&dfWi4=" & dtWino4 & "&dfWi5=" & dtWino5 & "&dfSeq1=" & dtSeq1 & "&dfSeq2=" & dtSeq2 & "&dfSeq3=" & dtSeq3 & "&dfSeq4=" & dtSeq4 & "&dfSeq5=" & dtSeq5 & "&dfLot=" & dtLot)
            If rsData <> "0" Then
                Return rsData
            Else
                Return "0"
    ...
    Public Shared Function mgroupDataWiSpc(WI As String, seq As String, dtLot As String)
        Try
            Dim requestFunction As New JObject()
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefectSpecial/groupDataWiSpc?dfWi=" & WI & "&dfseq=" & seq & "&dfLot=" & dtLot)
            If rsData <> "0" Then
                Return rsData
            Else
                Return "0"
    ...
    Public Shared Function Getdatachildpartsummaryfggrouppart(dtWino As String, dtSeq As String, dtLot As String, dfType As String)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/Getdatachildpartsummaryfggrouppart?dfWi=" & dtWino & "&dfSeq=" & dtSeq & "&dfLot=" & dtLot & "&dfType=" & dfType)
            If rsData <> "0" Then
                Return rsData
            Else
                Return "0"
    ...
    Public Shared Function mGetdatachildpartsummarychildgrouppart(dtWino As String, dtSeq As String, dtLot As String, dfType As String)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/Getdatachildpartsummarychildgrouppart?dfWi=" & dtWino & "&dfSeq=" & dtSeq & "&dfLot=" & dtLot & "&dfType=" & dfType)
            If rsData <> "0" Then
                Return rsData
            Else
                Return "0"
    ...
    Public Shared Function GetdatachildpartsummarychildgrouppartAdminAdjust(dtWino As String, dtSeq As String, dtLot As String, dfType As String, itemCd As String)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/GetdatachildpartsummarychildgrouppartAdminAdjust?dfWi=" & dtWino & "&dfSeq=" & dtSeq & "&dfLot=" & dtLot & "&dfType=" & dfType & "&ItemCd=" & itemCd)
            If rsData <> "0" Then
                Return rsData
            Else
                Return "0"
    ...
    Public Shared Function mGetdefectactual(LineCd As String, sDate As String, eDate As String)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/getDefectactualadmin?LineCd=" & LineCd & "&sDate=" & sDate & "&eDate=" & eDate)
            If rsData <> "0" Then
                Return rsData
            Else
                Return "0"
    ...
    Public Shared Function mGetmasterDataDefect(def_cd As String)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/getmasterDataDefect?def_cd=" & def_cd)
            Console.WriteLine("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/getmasterDataDefect?def_cd=" & def_cd)
            If rsData <> "0" Then
                Return rsData
            Else
                Return "0"
    ...
    Public Shared Function mGetDefectadmindetailncFG(LineCd As String, sDate As String, eDate As String)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/getDefectadmindetailncFG?LineCd=" & LineCd & "&sDate=" & sDate & "&eDate=" & eDate)
            Console.WriteLine("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/getDefectadmindetailncFG?LineCd=" & LineCd & "&sDate=" & sDate & "&eDate=" & eDate)
            If rsData <> "0" Then
                Return rsData
            Else
                Return "0"
    ...
    Public Shared Function mGetDefect(wi As String, lotNo As String, shift As String)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/GetDefect?wi=" & wi & "&lotNo=" & lotNo & "&shift=" & shift)
            Return rsData
        Catch ex As Exception
    ...
    Public Shared Function mGetPartBySelectDate(LineCd As String, sDate As String, Shift As String, Type As String)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/GetPartBySelectDate?LineCd=" & LineCd & "&sDate=" & sDate & "&Shift=" & Shift & "&Type=" & Type)
            Console.WriteLine("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/GetPartBySelectDate?LineCd=" & LineCd & "&sDate=" & sDate & "&Shift=" & Shift & "&Type=" & Type)
            If rsData <> "0" Then
                Return rsData
            Else
                Return "0"
    ...
    Public Shared Function mGetDataAdminAdjustByWi(LineCd As String, sDate As String, Shift As String, wi As String, itemType As String, seqNo As String, Type As String)
        Try
            ...
            'MsgBox("seqNo = " & seqNo)
            'MsgBox("Type = " & Type)
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/GetPartBySelectDateWi?LineCd=" & LineCd & "&sDate=" & sDate & "&Shift=" & Shift & "&Wi=" & wi & "&itemType=" & itemType & "&seqNo=" & seqNo & "&Type=" & Type)
            ' MsgBox(rsData)
            If rsData <> "0" Then
                Return rsData
            Else
    ...
    Public Shared Function mGetDefectadmindetailng(LineCd As String, sDate As String, eDate As String)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/getDefectadmindetailngFG?LineCd=" & LineCd & "&sDate=" & sDate & "&eDate=" & eDate)
            Console.WriteLine("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/getDefectadmindetailngFG?LineCd=" & LineCd & "&sDate=" & sDate & "&eDate=" & eDate)
            If rsData <> "0" Then
                Return rsData
            Else
                Return "0"
    ...
    Public Shared Function mGetQtyofdefectcode(dtWino As String, dtLotNo As String, dtSeqno As String, dtType As String, dtCode As String, itemCd As String)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/getQtyofdefectcode?dtWino=" & dtWino & "&dtLotNo=" & dtLotNo & "&dtSeqno=" & dtSeqno & "&dtType=" & dtType & "&dtCode=" & dtCode & "&ItemCd=" & itemCd)
            Return rsData
        Catch ex As Exception
    ...
    Public Shared Function mGetDatasys_exp_defect_mst(def_cd As String)
        Try
            Dim api = New api()
            Dim rsData = api.Load_data("http://" & Backoffice_model.svApi & "/apiShopfloor_gemba/getDatadefect/GetDatasys_exp_defect_mst?def_cd=" & def_cd)
            Return rsData
        Catch ex As Exception
