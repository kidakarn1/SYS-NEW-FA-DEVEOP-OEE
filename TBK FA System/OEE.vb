...
Public Class OEE
    ...
    Public Shared Function OEE_GET_Hour(shift As String)
        Dim api = New api()
        Dim TarGet = api.Load_data("http://" & Backoffice_model.svApi & "/API_NEW_FA/GET_OEE/GET_HOUR?shift=" & shift)
        Return TarGet
    End Function
    Public Shared Function OEE_getProduction_actual_detailByHour(line_cd As String)
        Dim api = New api()
        Dim rs = api.Load_data("http://" & Backoffice_model.svApi & "/API_NEW_FA/GET_OEE/getProduction_actual_detailByHour?line_cd=" & line_cd)
        Return rs
    End Function
    Public Shared Function GetDataProgressbarA(st_shift As String, end_shift As String, line_cd As String)
        Dim api = New api()
        Dim rs = api.Load_data("http://" & Backoffice_model.svApi & "/API_NEW_FA/GET_OEE/progressbarA?st_shift=" & st_shift & "&end_shift=" & end_shift & "&line_cd=" & line_cd)
        Console.WriteLine("http://" & Backoffice_model.svApi & "/API_NEW_FA/GET_OEE/progressbarA?st_shift=" & st_shift & "&end_shift=" & end_shift & "&line_cd=" & line_cd)
        Return rs
    End Function
    ...
    Public Shared Function OEE_GET_Data_LOSS(line_cd As String, lot_no As String, shift As String, dateStart As String)
        Dim api = New api()
        Dim TarGet = api.Load_data("http://" & Backoffice_model.svApi & "/API_NEW_FA/GET_OEE/GetDataAvailabillty?line_cd=" & line_cd & "&lot_no=" & lot_no & "&shift=" & shift & "&dateStart=" & dateStart)
        Console.WriteLine("http://" & Backoffice_model.svApi & "/API_NEW_FA/GET_OEE/GetDataAvailabillty?line_cd=" & line_cd & "&lot_no=" & lot_no & "&shift=" & shift & "&dateStart=" & dateStart)
        Return TarGet
    End Function
    Public Shared Function OEE_GET_Data_AccTarget(st_shift As String, std_ct As String)
        Dim api = New api()
        Dim AccTarGet = api.Load_data("http://" & Backoffice_model.svApi & "/API_NEW_FA/GET_OEE/getAccTarget?st_shift=" & st_shift & "&std_ct=" & std_ct)
        Return AccTarGet
    End Function
    Public Shared Function OEE_getSpeedLoss(NG As String, Good As String, Timeshift As String, std_cd As String)
        Dim api = New api()
        Dim AccTarGet = api.Load_data("http://" & Backoffice_model.svApi & "/API_NEW_FA/GET_OEE/getSpeedLoss?NG=" & NG & "&Good=" & Good & "&Timeshift=" & Timeshift & "&std_cd=" & std_cd)
